...
import { decodeB64, encodeB64 } from "./b64codec.js";
...
export class BlobCodex {
  ...
  serialize() {
    ...
    for (const i of Object.keys(this.keys).map(Number).sort()) {
      ...
      ikeys.push(i);
      keys.push(key);
      if (salt !== null) salt = encodeB64(salt);
      salts.push(salt);
    }
  ...
  static async hydrate(s, passwords, options = {}) {
    ...
    const salts = want.reduce((salts, i) => {
      salts[i] = s.salts[i] !== null ? decodeB64(s.salts[i]) : null;
      return salts;
    }, {});
  ...
  getItem(id, ikey = undefined) {
    ...
    for (const blob of item.blobs) {
      ...
      let data;
      if (key === null) {
        data = Buffer.from(decodeB64(blob.blob));
      } else {
        if (!key)
          throw new Error(
        ...
        const decipher = createDecipher(key, {
          iv: decodeB64(blob.params.iv),
          alg: blob.params.alg,
          tag: decodeB64(blob.params.tag),
        });
        data = decipher.update(decodeB64(blob.blob));
